...
public class ReminderSchedulerTimingSpecs : Akka.Hosting.TestKit.TestKit
{
    ...
    public async Task Nack_ShouldUseRetryBudgetAndExposeTerminalStatus()
    {
        ...
        var firstNack = await extension.NackAsync(first, "first failure");
        Assert.Equal(ReminderNackResponseCode.RetryScheduled, firstNack.ResponseCode);
        Assert.Equal(1, firstNack.AttemptCount);

        var pending = await extension.GetOccurrenceStatusAsync(first.Entity, first.Key, first.DueTimeUtc);
        ...
        var secondNack = await extension.NackAsync(second, "second failure");
        ...
        var thirdNack = await extension.NackAsync(third, "final failure");
        Assert.Equal(ReminderNackResponseCode.Failed, thirdNack.ResponseCode);
        Assert.Equal(3, thirdNack.AttemptCount);

        var failed = await extension.GetOccurrenceStatusAsync(third.Entity, third.Key, third.DueTimeUtc);
